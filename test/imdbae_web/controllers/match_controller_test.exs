defmodule ImdbaeWeb.MatchControllerTest do
  use ImdbaeWeb.ConnCase

  alias Imdbae.Social
  alias Imdbae.Social.Match

  @create_attrs %{email1: "some email1", email2: "some email2"}
  ...
  def fixture(:match) do
    {:ok, match} = Social.create_match(@create_attrs)
    match
  end
  ...
  describe "update match" do
    ...
    test "renders match when data is valid", %{conn: conn, match: %Match{id: id} = match} do
      ...
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "email1" => "some updated email1",
        "email2" => "some updated email2"}
    end
